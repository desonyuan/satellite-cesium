...
void initspw(spwdata spwarr[spwsize], double& jdspwstart)
	 {
      ...
      //  infile = fopen("sw_2025.txt", "r");
       infile = fopen("D:\\HPOP_code\\HPOP_RK4\\sw_2025.txt", "r");

       // ---- read number of data points
       char tline[256];  // Array to store each line read from the file
   
       // Read the file line by line
       while (fscanf(infile, "%255[^\n]\n", tline) != EOF) {
           // Check if the line contains the string "NUM_OBSERVED_POINTS"
           if (strstr(tline, "NUM_OBSERVED_POINTS") != NULL) {
               // Use sscanf to extract the number after "NUM_OBSERVED_POINTS"
               sscanf(tline, "NUM_OBSERVED_POINTS %d", &numrecsobs);
               break;  // Exit the loop after finding the required line
           }
       }
       ...
       for (i = numrecsobs; i < numrecsobs + numrecspred; i++)
         {
           ...
           fscanf(infile,"%4i %3d %3d %5i %3i %3i %3i %3i %3i %3i %3i %3i %3i %4i %4i %4i %4i %4i %4i %4i %4i %4i %4i %4lf %2i %4i %6lf %6lf %6lf %6lf %6lf %6lf \n",
                     ...
                     &spwarr[i].aparr[0],&spwarr[i].aparr[1],&spwarr[i].aparr[2],&spwarr[i].aparr[3],
                     &spwarr[i].aparr[4],&spwarr[i].aparr[5],&spwarr[i].aparr[6], &spwarr[i].aparr[7], &spwarr[i].avgap,
                     &spwarr[i].cp, &spwarr[i].c9, &spwarr[i].isn, &spwarr[i].adjf10,
                     &spwarr[i].adjctrf81, &spwarr[i].adjlstf81, &spwarr[i].obsf10, &spwarr[i].obsctrf81, &spwarr[i].obslstf81 );
					 spwarr[i].q = 0;
         }
      fclose( infile );
...
void initeop
	 ...
	 )
	 {
      ...
      //  infile  = fopen("EOP-All_2025.txt", "r");
       infile  = fopen("D:\\HPOP_code\\HPOP_RK4\\EOP-All_2025.txt", "r");
