...
extern int dat;
extern double jdeopstart,dut1,lod,xp,yp,ddpsi,ddeps,dx,dy,x,y,s,deltapsi,deltaeps;
extern eopdata eoparr[eopsize];
...
const double IERS::TT_GPS  = IERS::TT_TAI - IERS::GPS_TAI;  // TT-GPS time difference [s]
...
double IERS::TAI_UTC(double Mjd_UTC) { return -UTC_TAI_;}
...
{
  return Modulo ( GMST(Mjd_UT1) + EqnEquinox(Mjd_TT), pi2 );
}
...
//
//------------------------------------------------------------------------------
Matrix GHAMatrix(double Mjd_UT1, double Mjd_TT)
{
  return R_z( GAST(Mjd_UT1, Mjd_TT) );
}
...
//
//  Mjd_UTC     Modified Julian Date(UTC)
//  Y0          Satellite's state vector in ECEF coodinate system
//
// Outputs:
//
//  r           Satellite's position vector in ECI coodinate system
//  v           Satellite's velocity vector in ECI coodinate system
//
//--------------------------------------------------------------------------
Vector ECEF2ECI(double Mjd_UTC, const Vector& Y0)
{
  double Mjd_UT1, Mjd_TT, jd, mfme, Omega;
  Matrix P(3,3), N(3,3), Theta(3,3), Pi(3,3), S(3,3), dTheta(3,3), U(3,3), dU(3,3);
  Vector r(3), v(3);
  char interp = 'l';
  ...
  S(0,1) = 1;
  S(1,0) = -1;
  //Omega = 7292115.8553e-11+4.3e-15*( (Mjd_UTC-MJD_J2000)/36525.0 ); // [rad/s]
  Omega  = omega_Earth-0.843994809*1e-9*lod; // [rad/s] IERS
  ...
  r = Transp(U)*Y0.slice(0,2);
  v = Transp(U)*Y0.slice(3,5) + Transp(dU)*Y0.slice(0,2);
...
//--------------------------------------------------------------------------
//
// ECI2ECEF: Transforms Earth Centered Earth Fixed (ECI) coordinates to
//           Earth Centered Inertial (ECEF) coordinates
//
// Inputs:
//
//  Mjd_UTC     Modified Julian Date(UTC)
//  Y0          Satellite's state vector in ECI coodinate system
//
// Outputs:
//
//  r           Satellite's position vector in ECEF coodinate system
//  v           Satellite's velocity vector in ECEF coodinate system
//
//--------------------------------------------------------------------------
Vector ECI2ECEF(double Mjd_UTC, const Vector& Y0)
{
  double Mjd_UT1, Mjd_TT, jd, mfme, Omega;
  Matrix P(3,3), N(3,3), Theta(3,3), Pi(3,3), S(3,3), dTheta(3,3), U(3,3), dU(3,3);
  Vector r(3), v(3);
  ...
  // Transformation from WGS to ICRS
  r = U*Y0.slice(0,2);
  v = U*Y0.slice(3,5) + dU*Y0.slice(0,2);
